...
public class Utils {
	...
	public Conditions getConditions(DateTime time, DateTime date2) {
		...
		System.out.println("现在是"+time.getHourOfDay()+"点");
		if(time.getHourOfDay()>=0&&time.getHourOfDay()<12){
		conditions.setNotBefore(time.minusHours(4).plusDays(1));
		conditions.setNotOnOrAfter(date2.minusHours(4).plusDays(1));
		}
		else {
